...
combine_for_python <- function() {
  ...
  rna_features_matched  <- rna_features_matched[final_labels$cell_id, ]
  atac_features_matched <- atac_features_matched[final_labels$cell_id, ]
  # NOTE: all three feature matrices now share an identical row set
  # (final_labels$cell_id), so accuracy differences across the three
  # downstream Python runs are attributable to feature MODALITY alone,
  # not to differing sets of cells.
  ...
  # ----------------------------------------
  # We export THREE feature matrices to support a modality ablation
  # (responding to reviewer concerns about label leakage from RNA into a
  # supervised classifier whose labels were themselves derived from RNA):
  #
  #   1. combined_features.csv   -- RNA (2000) + ATAC (5000) [original]
  #   2. rna_only_features.csv   -- RNA only (ceiling / leakage upper bound)
  #   3. atac_only_features.csv  -- ATAC only (honest test of the
  #                                  chromatin-accessibility -> cell type claim)
  #
  # All three share identical cells (`common_cells`) and identical labels
  # (`final_labels`), so accuracy differences across the three runs are
  # attributable to feature modality alone.
  write.csv(combined_features,     file.path(output_dir_python, "combined_features.csv"))
  write.csv(rna_features_matched,  file.path(output_dir_python, "rna_only_features.csv"))
  write.csv(atac_features_matched, file.path(output_dir_python, "atac_only_features.csv"))
  # (Keep the legacy filenames too for backward compatibility with older scripts)
